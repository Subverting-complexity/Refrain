...
| `docs/RELEASING.md`          | The `Deploy` entry point, platform and lane selection, EAS auth, the pull-request version bump, release-branch/outcome-tag tracking, the listing push, and store-submission troubleshooting. Read before touching `tools/ps/Deploy.ps1`, `tools/version-bump.mjs` or `tools/release-branch.mjs`. |
